...
local WantedItems = {
    "Rare Box","Ultra Rare Box","Chilly Fruit","Vampire Fruit","Sand Fruit",
    "Plasma Fruit","Snow Fruit","Light Fruit","Candy Fruit","Quake Fruit",
    "Blood Fruit","Ope Fruit","Gum Fruit","Dark Fruit","Rumble Fruit",
    "Gravity Fruit","Venom Fruit","Magma Fruit","Gas Fruit","Hollow Fruit",
    "Flare Fruit","Alice Fruit","Reset Token","Compass",
}

local WantedSet = {}
for _, n in ipairs(WantedItems) do WantedSet[n] = true end
...
local function IsWantedTool(obj)
    ...
    for _, name in ipairs(WantedItems) do
    ...
    if bp then
        for _, item in ipairs(bp:GetChildren()) do
            if item:IsA("Tool") then
                for _, name in ipairs(WantedItems) do
                    if item.Name:find(name) then return item.Name end
                end
            end
        end
    end
...
-- ใส่ label เดียวแทน loop (ป้องกัน Fluent พัง)
Tabs.Item:AddSection("รายการของ")
local sortedItems = {}
for _, n in ipairs(WantedItems) do table.insert(sortedItems, n) end
table.sort(sortedItems)
Tabs.Item:AddLabel(table.concat(sortedItems, " | "))

-- ─── Player Tab ───────────────────────────────────────────────────────────────
Tabs.Player:AddSection("Follow Player")
...
Tabs.Player:AddButton({
    ...
    Callback = function()
        ...
        if not targetPlr or not targetPlr.Character then
            Fluent:Notify({ Title = "Error", Content = "ไม่พบ Player", Duration = 3 })
            return
        end
        local targetRoot = GetModelRoot(targetPlr.Character)
        if targetRoot then
...
Tabs.Teleport:AddButton({
    ...
    Callback = function()
        ...
        if target and hrp then
            ...
            Fluent:Notify({ Title = "ไม่พบ Monster", Content = "ไม่มีมอนที่ผ่านเงื่อนไข", Duration = 3 })
        end
    end,
})
