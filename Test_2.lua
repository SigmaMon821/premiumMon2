...
local function LoadMainScript()
    ...
    Farm = Window:AddTab({ Title = "Auto Farm", Icon = "hammer" }),
   Shop = Window:AddTab({ Title = "Shop", Icon = "shopping-cart" }),
   Skill = Window:AddTab({ Title = "Auto Skill", Icon = "swords" }),
    ...
    Server = Window:AddTab({ Title = "Server", Icon = "globe" }),
       ...
       local keys = {"Z", "X", "C", "V", "B", "N", "F", "G", "H", "J", "K", "L"}
    local active = {}

    for _, key in ipairs(keys) do
        active[key] = false
        Tabs.Skill:AddToggle("Auto" .. key, {Title = "Auto " .. key, Default = false}):OnChanged(function(value)
            active[key] = value
        end)
    end
    ...
    local AutoFarmHaki = Tabs.Main:AddToggle("AutoFarmHakiToggle", {
    Title = "Auto Farm Haki (op)",
    Default = false
})

AutoFarmHaki:OnChanged(function()
        ...
        task.spawn(function()
            while Options.AutoFarmHakiToggle.Value do
                pcall(function()
                    ...
                    local targetRemote = workspace.UserData["User_" .. tostring(player.UserId)].III
...
Tabs.Main:AddToggle("AutoSpawn", {Title = "Auto Spawn", Default = false}):OnChanged(function(v)
    ...
    if _G.AutoSpawnEnabled then
        task.spawn(function()
            while _G.AutoSpawnEnabled do
                ...
                pcall(function()
                    local player = game.Players.LocalPlayer
                    local loadFrame = player.PlayerGui:FindFirstChild("Load") and player.PlayerGui.Load:FindFirstChild("Frame")
                    
                    if loadFrame and loadFrame.Visible then
                        local button = loadFrame:FindFirstChild("Load")
                        if button then
                            if firesignal then
    ...
    ["Venom Fruit"] = true, ["Magma Fruit"] = true, ["Gas Fruit"] = true, ["Hollow Fruit"] = true, 
    ["Flare Fruit"] = true, ["Luck Fruit"] = true, ["Alice Fruit"] = true, 
}
...
Tabs.Main:AddToggle("AutoPickup", {Title = "Auto Pickup ", Default = false}):OnChanged(function(v)
    autoPickup = v
    if v then 
        task.spawn(function()
            while autoPickup do
                ...
                if hrp then
                    ...
                    if collectedSomething and hrp then 
...
local Section = Tabs.Shop:AddSection("Drink Utilities")

Tabs.Shop:AddButton({
    Title = "Instant Drink",
    Description = "Consumes all drinks in backpack immediately",
    Callback = function()
...
-- ===== DROP DRINK ITEMS =====
local Section_Drop = Tabs.Shop:AddSection("Drop Drink Items")

local autoDropDrinks = false
local dropDrinkThread = nil

local function dropDrinkItems()
    ...
    for _, v in pairs(backpack:GetChildren()) do
        if v:IsA("Tool") and (
            string.find(v.Name, "Juice")
            or string.find(v.Name, "Cider")
            or string.find(v.Name, "Lemonade")
            or string.find(v.Name, "Smoothie")
        ) then
            table.insert(toDrop, v)
        end
    end
    -- ดรอปของที่ถืออยู่ด้วย
    if char then
    ...
    local count = #toDrop
    if count > 0 then
        Fluent:Notify({Title = "Drop Complete", Content = "Dropped " .. count .. " drink(s)!", Duration = 3})
    else
        Fluent:Notify({Title = "Drop", Content = "No drink items found in backpack.", Duration = 3})
    end
end
...
Tabs.Shop:AddButton({
    ...
    Callback = function()
        ...
        local msg = #items > 0 and table.concat(items, ", ") or "Backpack is empty"
        Fluent:Notify({
...
-- ===== END DROP DRINK ITEMS =====

local Section = Tabs.Shop:AddSection("Auto Buy Drinks")

local selectedDrinks = {} 
local autoBuyEnabled = false 
local drinkQuantity = 1 

local MultiDrinkDropdown = Tabs.Shop:AddDropdown("MultiDrinkDropdown", {
    Title = "Select Drinks",
...
MultiDrinkDropdown:OnChanged(function(value)
    selectedDrinks = value 
end)

Tabs.Shop:AddInput("QuantityInput", {
    Title = "Set Drink Quantity",
    Description = "Enter amount to buy per drink",
    Default = "1",
    Placeholder = "Enter quantity...",
    Numeric = true,
    Finished = false,
    Callback = function(value)
        drinkQuantity = tonumber(value) or 1
    end
})
                ...
                pcall(function()
                    if next(selectedDrinks) then
                        for drink, isSelected in pairs(selectedDrinks) do
                            if isSelected and autoBuyEnabled then
                                ...
                                for _ = 1, drinkQuantity do
                                    ...
                                    if merchants then
                                        local npc = merchants:FindFirstChild("BetterDrinkMerchant")
                                        if npc and npc:FindFirstChild("Clickable") and npc.Clickable:FindFirstChild("ClickDetector") then
                                    ...
                                    if playerGui:FindFirstChild("ShopDrinksPlus") and playerGui.ShopDrinksPlus:FindFirstChild("Clicked") then
                                        local args = {
                                            [1] = drink
                                        }
                        ...
                        Fluent:Notify({
                            ...
                            Content = "Purchase sequence finished",
                            Duration = 3
                        })
                    end
                end)
...
local standCFrame = CFrame.new(-3133.87, 557.86, -4092.61)

local targetPoints = {
    Vector3.new(-2643.20, 402.59, -4465.90),
    Vector3.new(-3492.67, 402.59, -4527.46),
    Vector3.new(-3673.16, 401.91, -3814.21),
    Vector3.new(-2605.10, 334.78, -3769.86)
}

_G.AutoGravitySkill = false
local positionLock

local function lockPlayerPosition()
    if positionLock then positionLock:Disconnect() end
    positionLock = RunService.Heartbeat:Connect(function()
        if _G.AutoGravitySkill and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            player.Character.HumanoidRootPart.CFrame = standCFrame
        elseif not _G.AutoGravitySkill then
...
Tabs.Farm:AddToggle("AutoSkillGravity", {
    Title = "Auto Farm Gravity", 
    Default = false,
    Callback = function(state)
        _G.AutoGravitySkill = state
        ...
        if state then
            ...
            task.spawn(function()
                local index = 1
                while _G.AutoGravitySkill do
                    local character = player.Character
                    if character and character:FindFirstChild("Powers") and character.Powers:FindFirstChild("Gravity") then
                        local remote = character.Powers.Gravity.RemoteEvent
                        local targetPos = targetPoints[index]
                        local targetCFrame = CFrame.new(targetPos)
                        
                        remote:FireServer("GravityPower12", "StartCharging")
                        remote:FireServer("GravityPower12", "StartCharging", targetCFrame)
                        
                        task.wait(chargeTime)
                        
                        if _G.AutoGravitySkill then
                            remote:FireServer("GravityPower12", "StopCharging", targetCFrame, nil, 300)
                        end
                        
                        task.wait(cooldownTime)
                        
                        index = index + 1
                        if index > #targetPoints then
                            index = 1
                        end
                    else
                        task.wait(1)
...
local selectedSmelt = "Cave Diamond" 
local isAutoSmeltOn = false
local smeltThread = nil
local smeltNoclipConnection = nil

local function enableSmeltNoclip()
    if smeltNoclipConnection then return end
    smeltNoclipConnection = RunService.Stepped:Connect(function()
        local char = player.Character
        if char then
...
local function disableSmeltNoclip()
    if smeltNoclipConnection then
        smeltNoclipConnection:Disconnect()
        smeltNoclipConnection = nil
    end
end

local function updateSmeltNoclip()
    if isAutoSmeltOn then
        if selectedSmelt == "Vokun" then
            disableSmeltNoclip()
        else
            enableSmeltNoclip()
        end
...
local function loopSmelt()
    local char = player.Character
    local rootPart = char and char:FindFirstChild("HumanoidRootPart")
    if rootPart then
        if selectedSmelt == "Cave Diamond" then
            rootPart.CFrame = CFrame.new(2211.09, 216.00, -632.65)
        elseif selectedSmelt == "Boss Gunsling" then
            rootPart.CFrame = CFrame.new(-1107.25, 218.00, 1595.78)
        elseif selectedSmelt == "Vokun" then
            rootPart.CFrame = CFrame.new(4570.93, 217.40, 5164.01)
        end
        task.wait(0.5)
    end

    while isAutoSmeltOn do
        pcall(function()
            local char = player.Character
            if char and char:FindFirstChild("Powers") and char.Powers:FindFirstChild("Smelt") then
                local remote = char.Powers.Smelt:FindFirstChild("RemoteEvent")
                if not remote then return end

                if selectedSmelt == "Cave Diamond" then
                    local targetStone = workspace:FindFirstChild("IslandMountain") and workspace.IslandMountain:FindFirstChild("Stone") and workspace.IslandMountain.Stone:FindFirstChild("Stone")
                    if targetStone then
                        remote:FireServer("SmeltPower3", "Attempt", CFrame.new(2207, 217.178, -632.607) * CFrame.Angles(-2.142, 1.216, 2.171), targetStone)
                    end
                elseif selectedSmelt == "Boss Gunsling" then
                    local targetStone = workspace:FindFirstChild("IslandCaver") and workspace.IslandCaver:FindFirstChild("Stones") and workspace.IslandCaver.Stones:FindFirstChild("Stone")
                    if targetStone then
                        remote:FireServer("SmeltPower3", "Attempt", CFrame.new(-958.5, 218.592, 1702.979) * CFrame.Angles(-3.114, -0.953, -3.119), targetStone)
                    end
                elseif selectedSmelt == "Vokun" then
                    local targetField = workspace:FindFirstChild("Enemies") and workspace.Enemies:FindFirstChild("Lv2000 Vokun") and workspace.Enemies["Lv2000 Vokun"]:FindFirstChild("VokunField") and workspace.Enemies["Lv2000 Vokun"].VokunField:FindFirstChild("VokunField") and workspace.Enemies["Lv2000 Vokun"].VokunField.VokunField:FindFirstChild("Field")
                    if targetField then
                        remote:FireServer("SmeltPower3", "Attempt", CFrame.new(4571.593, 220.495, 5181.448) * CFrame.Angles(3.053, -0.040, 3.138), targetField)
                    end
                end
            end
        end)
...
local SmeltDropdown = Tabs.Farm:AddDropdown("SmeltSelect", {
    Title = "Select Smelt Target",
    Values = {"Cave Diamond", "Boss Gunsling", "Vokun"},
    Multi = false,
    Default = 1,
})

SmeltDropdown:OnChanged(function(Value)
    selectedSmelt = Value
    if isAutoSmeltOn then
        updateSmeltNoclip()
        if smeltThread then task.cancel(smeltThread) end
        smeltThread = task.spawn(loopSmelt)
    end
end)

local SmeltToggle = Tabs.Farm:AddToggle("AutoSmeltToggle", {
    Title = "Auto Smelt",
    Default = false
})

SmeltToggle:OnChanged(function(Value)
    isAutoSmeltOn = Value
    updateSmeltNoclip()
    if isAutoSmeltOn then
        if smeltThread then task.cancel(smeltThread) end
...
-----จบของ tab Auto Fram 
    local tpLocations = {
        ...
        ["Large island"] = CFrame.new(-6474.30, 276.00, 4519.80),
        ["Marine Ford"] = CFrame.new(-3134.41, 557.56, -4090.88),
        ["Gunsling island"] = CFrame.new(-1076.26, 361.00, 1702.01),
        ["Snow Island"] = CFrame.new(-1899.79, 225.00, 3292.03),
        ["IslandTREEABIGHOUSE"] = CFrame.new(1107.45, 217.20, 3353.74),
        ["IslandCrescent"] = CFrame.new(3185.51, 217.00, 1526.23),
        ["Island12"] = CFrame.new(4832.99, 570.00, -7127.42)
...
local function hookHumanoid(character)
    ...
    if humanoid then
        ...
        wsConnection = humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
            if loopWs and humanoid.WalkSpeed ~= wsVal then
...
Tabs.Misc:AddToggle("WalkSpeedToggle", {Title = "Loop WalkSpeed", Default = false}):OnChanged(function(v)
    loopWs = v
...
Tabs.Misc:AddInput("WSInput", {
    ...
    Finished = true,
    Callback = function(v)
        wsVal = tonumber(v) or 16
        if loopWs then
            local hum = plr.Character and plr.Character:FindFirstChild("Humanoid")
            if hum then hum.WalkSpeed = wsVal end
        end
    end
})
...
rs.Stepped:Connect(function()
    if loopWs then
        local hum = plr.Character and plr.Character:FindFirstChild("Humanoid")
        if hum and hum.WalkSpeed ~= wsVal then
            hum.WalkSpeed = wsVal
        end
    end
end)
...
Tabs.Misc:AddButton({
    ...
    Callback = function()
        pcall(function()
            loadstring(game:HttpGet("https://pastebin.com/raw/GWxt3bBb"))()
        end)
    end
})
...
Tabs.Misc:AddButton({
    ...
    Callback = function()
        ...
        if req then
            local url = "https://games.roblox.com/v1/games/" .. tostring(game.PlaceId) .. "/servers/Public?sortOrder=Desc&limit=50"
