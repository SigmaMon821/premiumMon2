...
-- ใช้ ClassName = Folder หา Alive
local function GetAliveFolder()
    for _, obj in ipairs(workspace:GetChildren()) do
        if obj:IsA("Folder") and obj.Name == "Alive" then
            return obj
        end
    end
    return nil
...
-- ใช้ ClassName = Tool หาของใน workspace
...
-- CFrame offset ตามโหมด
local function GetBehindCFrame(rootCF, offset)
    return rootCF * CFrame.new(0, 0, offset)
end

local function GetAboveCFrame(rootCF, offset)
    return rootCF * CFrame.new(0, offset, 0) * CFrame.Angles(math.pi, 0, 0)
end

local function GetOffsetCFrame(rootCF, mode, offset)
    if mode == "Behind" then
        return GetBehindCFrame(rootCF, offset)
    else
        return GetAboveCFrame(rootCF, offset)
    end
end

-- ─── Auto Farm (Heartbeat lock-on) ───────────────────────────────────────────
...
local function StartFarm()
    ...
    -- Heartbeat: ประกบตำแหน่งมอนทุก frame
    State.FarmConnection = RunService.Heartbeat:Connect(function()
        ...
        -- ตรวจว่า target ยังมีชีวิตอยู่ไหม
    ...
    -- Attack loop แยกต่างหาก (ไม่ผูกกับ Heartbeat เพื่อไม่ให้ถี่เกิน)
...
-- ─── Item ESP (ClassName: Tool ใน Backpack / Character) ──────────────────────
local function CheckPlayerItems(plr)
    ...
    -- หาใน Character (Tool ที่กำลังถืออยู่)
    if not found then
...
-- ─── Auto Pickup (ClassName: Tool ใน workspace) ───────────────────────────────
local function StartAutoPickup()
    task.spawn(function()
        while State.AutoPickup do
            ...
            local hrp = GetHRP()
            if hrp then
                -- วนหา Tool (ClassName) ใน workspace โดยตรง
                for _, obj in ipairs(workspace:GetChildren()) do
                    if not State.AutoPickup then break end
                    if IsWantedTool(obj) then
                        local handle = obj:FindFirstChild("Handle")
                        if handle and not obj.Parent:FindFirstChildOfClass("Humanoid") then
...
-- ─── Follow Player (Heartbeat lock-on) ───────────────────────────────────────
...
Tabs.Item:AddToggle("ItemESPToggle", {
    Title       = "Item ESP",
    Description = "แสดงชื่อ Player ที่มีของสำคัญ (ค้นจาก ClassName: Tool)",
    Default     = false,
    Callback    = function(val)
        State.ItemESP = val
...
Tabs.Item:AddToggle("AutoPickupToggle", {
    Title       = "Auto Pickup",
    Description = "วาปเก็บ ClassName: Tool ใน workspace อัตโนมัติ",
    Default     = false,
    Callback    = function(val)
